...
VariableGBFRecord::VariableGBFRecord(GBFTable* table, uint record_index) : table_data_(table), current_record_(record_index), valid_(true) {
    ...
    id_ = readlong(buffer_, 13 + current_record_ * 13);
}

ErrorCode VariableGBFRecord::next() {
...
byte* VariableGBFRecord::getRecordBuffer() {
    if (!valid_) return nullptr;
    uint record_offset = readint(buffer_, 21 + current_record_ * 13);
    return const_cast<byte*>(buffer_) + record_offset;
}

ErrorCode VariableGBFRecord::getField(const char* target_name, void* out, uint out_len) {
    if (!valid_) return ErrorCode::E_INVALID;
...
ErrorCode VariableGBFRecord::handleField(byte field_type, byte** record_buffer_ptr, void* out, uint out_len, bool want_output) {
